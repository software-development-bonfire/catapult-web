...
use App\Services\KDS\KDSTransactionV2Service;
use App\Services\KDS\TerminalTransactionService;
use App\Services\KDS\StoreOrderService;
...
class KDSStoreOrderController extends KDSBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $transactions = app()->make(StoreOrderService::class)->storeCreateUpdate($request->transaction);

        // Step 2: Construct KDS data and persist to KDS tables
        if (!empty($transactions)) {
            $kdsData = app()->make(KDSTransactionV2Service::class)->store($transactions);
