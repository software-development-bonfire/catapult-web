...
class StickerPrinter
{
    ...
    public function getQrcodeCommand($text, $x, $y, $correction = 'H', $cellWidth = 4, $mode = 'A', $rotation = 0)
    {
        $str = self::QRCODE;
        $str .= self::SPACE;
        $str .= $x;
    ...
    public function text($text, $x = 0, $y = 0, $font = 0, $rotation = 0, $xMultiplication = 0, $yMultiplication = 0, $alignment = null, $includeSetup = false)
    {
        array_push($this->commands, $this->getTextCommand($x, $y, $font, $rotation, $xMultiplication, $yMultiplication, $alignment, $text));
    }

    public function barcode($text, $x, $y, $type = '128', $height = 50, $humanReadable = 1, $rotation = 0, $narrow = 2, $wide = 2, $alignment = 2)
    {
        array_push($this->commands, $this->getBarcodeCommand($x, $y, $type, $height, $humanReadable, $rotation, $narrow, $wide, $alignment, $text));
    } 
    
    public function qrcode($text, $x, $y, $correction = 'H', $cellWidth = 4, $mode = 'A', $rotation = 0)
    {
        array_push($this->commands, $this->getQrcodeCommand($text, $x, $y, $correction, $cellWidth, $mode, $rotation));
    }

    public function image(TsplImage $image, $x, $y, $mode)
    {
        $image->toRasterFormat();
        array_push($this->commands, 'AZTEC 10,10,0,"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"'); // $this->getBitmapCommand($x, $y, $image->getWidthBytes(), $image->getHeight(), $mode, $image->toRasterFormat()));
    }

    public function setup($unit = 'mm', $width = '35', $height = '25', $gapDistance = 5, $gapOffset = 0, $speed = 4, $referenceX = 0, $referenceY = 0, $direction = 1)
    {
        $this->setSize($width, $height, $unit);
        $this->setGap($gapDistance, $gapOffset, $unit);
        $this->setSpeed($speed);
        $this->setReference($referenceX, $referenceY);
        $this->setDirection($direction);
        $this->setTear(true);
        $this->setShift(0, 0);

        array_push($this->commands, ...$this->getSetupCommands());
        $this->sendCommands($this->commands);
    }

    public function print($beep = true)
    {
        if ($beep) {
            array_push($this->commands, $this->getBeepCommand(9, 100));
        }
        array_push($this->commands, $this->getPrintCommand(1, 1));
        $this->sendCommands($this->commands);
        $this->clear();
    }
