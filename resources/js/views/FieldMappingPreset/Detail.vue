<template>
    <div class="module-container">
        <div class="box-row box-row--white p-1 d-flex justify-content-between align-content-center">
            <a href="/field-mapping-preset" class="back-to-list ml-2">
        ...
        <div class="container-fluid">
            <table class="table-layout pull-left col-xl-4">
                <tr>
                    ...
                    <td width="200px">
                        <select :disabled="mode === 'update' || form.mode === 'update'" class="form-control" v-model="form.values.type" @change="getSyncEntryChosen(); getPreset()">
                ...
                <tr>
                    ...
                    <td>
                        <select
                            class="form-control"
                            v-model="form.values.data_entry"
                        ...
                        <label
                            ...
                            v-if="errors.add.hasOwnProperty('data_entry')"
                        >
                            {{ errors.add.data_entry[0] }}
                        </label>
                    </td>
                </tr>
            </table>
                ...
                <tr>
                    ...
                    <td>
                        <input
                            type="text"
                            class="form-control"
                            :class="{ 'is-invalid': errors.error.hasOwnProperty('preset_name') }"
                            v-model="form.values.preset_name">
                        <label
                            class="text-danger error-message mb-0"
                            v-if="errors.add.hasOwnProperty('preset_name')"
                        >
                            {{ errors.add.preset_name[0] }}
                        </label>
                    </td>
                </tr>
                <tr v-if="this.form.values.bid || bid">
                    ...
                    <td>
                        <select class="form-control" v-model="form.values.copy_preset_from">
                            <option value=""></option>
                            <option :value="preset.bid" v-for="(preset, index) in presets" :key="index">{{preset.preset_name}}</option>
                        </select>
                    </td>
                    <td>
        ...
        <datatable
            ...
            class="
                ...
                datatable--font-sm"
            :header-fields="form.values.type === 1 ? table.cdis_to_pos.header : table.pos_to_cdis.header"
                ...
                <table-row
                    ...
                    <table-data>
                        <input
                            type="text"
                            class="form-control"
                            v-model="table.add.head_reference"
                            disabled>
                    </table-data>
                </table-row>
            </template>
    ...
    export default {
        ...
        async mounted() {
            ...
            if (searchParams.bid) {
                ...
                this.form.values.bid = Number(searchParams.bid);
                this.form.values.type = this.detail.type;
                this.form.values.data_entry = this.detail.data_entry;
                this.form.values.preset_name = this.detail.preset_name;
                this.form.values.version_status = Number(this.detail.status);
                if (this.detail.detail !== undefined) {
                    this.table.values.data = this.detail.detail.map(element => {
                        return {
                            edit: false,
                            id: element.id,
                            bid: element.bid,
        ...
        methods: {
                ...
                if (this.form.values.copy_preset_from) {
                    ...
                    this.dialog.ok.function = () => {
                        if (this.form.mode === 'create') {
                            ...
                            presets.forEach(element => {
                                this.table.values.data.push({
                                    ...
                                    default_value: element.default_value,
                                    column_name: element.column_name,
                                    head_reference: element.head_reference,
                                });
                            })
                        }
                        if (this.form.mode === 'update') {
                            ...
                            axios.post('/field-mapping-preset/detail/preset', config)
                            .then(response => {
                                this.table.values.data= [];
            ...
            setMapping() {
                var config = {
                    method: 'create',
                    type: this.form.values.type,
                    data_entry: this.form.values.data_entry,
                    preset_name: this.form.values.preset_name,
                    status: this.form.values.version_status,
                }

                axios.post('/field-mapping-preset/store', config)
                    .then(response => {
            ...
            save() {
                if (this.form.mode === 'create') {
                    ...
                    axios.post('/field-mapping-preset/details', config)
                    .then(response => {
                        this.dialog.visible = true;
                        ...
                        this.dialog.ok.function = () => {
                            this.dialog.visible = false;
                            window.open('/field-mapping-preset', '_self');
                        };
                        this.errors.add = {};
                        this.errors.add.field = '';
                    }).catch(error => {
                    ...
                    axios.patch(`/field-mapping-preset/update/${this.form.values.bid}`, config)
                    .then(response => {
                        this.dialog.visible = true;
                        this.dialog.status = 'success';
                        this.dialog.message = this.$t('success.successfully_updated', { value: this.$t('label.field_mapping_preset') });
                        this.dialog.ok.function = () => {
                            this.dialog.visible = false;
                            window.open('/field-mapping-preset', '_self');
                        };
            ...
            addRow() {
                if (this.form.mode === 'create') {
                    ...
                    var data = {
                        ...
                        default_value: this.table.add.default_value === "" ? '""' : this.defaultValue(this.table.add.default_value, this.table.add.mapping_type),
                        column_name: this.table.add.column_name,
                        head_reference: this.table.add.head_reference,
                    }

                    axios.post('/field-mapping-preset/detail/store', data)
                    .then(response => {
                        this.table.values.data.push({
                            edit: false,
                            ...
                            head_reference: response.data.data.head_reference,
                        });
                        this.clearFields();
            ...
            updateRow(data) {
                if (this.form.mode === 'create') {
                    ...
                    axios.patch(`/field-mapping-preset/detail/update/${data.values.bid}`, config)
                    .then(response => {
                        data.done();
                        this.table.values.data[data.rowIndex].edit = false,
                        this.table.values.data[data.rowIndex].bid = data.values.bid,
            ...
            deleteRow(data) {
                if (this.form.mode === 'create') {
                    ...
                    axios.delete(`/field-mapping-preset/detail/${data.values.bid}`)
                        .then(response => {
                            this.table.values.data.splice(data.rowIndex, 1);
                            this.dialog.status = 'success';
                            this.dialog.message = this.$t('success.successfully_removed_the_data');
